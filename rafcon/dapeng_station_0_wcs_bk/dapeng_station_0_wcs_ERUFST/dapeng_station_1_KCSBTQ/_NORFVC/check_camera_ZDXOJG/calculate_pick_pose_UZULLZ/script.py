...
def execute(self, inputs, outputs, gvm):
    ...
    pick_item_id = inputs["pick_box_id"]  
    #判断顶置相机是否已经识别到箱子  
    update_box_ids = gvm.get_variable("update_box_ids", per_reference=False, default=[])
    gvm.set_variable("update_box_ids", [], per_reference=False)  
    if pick_item_id in update_box_ids:
        self.logger.info(f"顶置相机已识别出目标箱子")
        return "no_camera"
    ...
    place_ws_id = inputs["place_ws_id"]
