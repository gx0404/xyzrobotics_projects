...
def execute(self, inputs, outputs, gvm):
    ...
    if workspace_type == u'conveyor':
        ...
        if enable_place_pose_from_hmi['place_pose_by_size']:
            ...
            # scan_code = gvm.get_variable("conveyor_pallet", per_reference=False, default=None)
            from xyz_logistics_hmi_back.utils.design import query_pallet_design_by_scan_code
            pallet_design_data = query_pallet_design_by_scan_code(scan_code, enable_place_pose_from_hmi['pallet_id'])
            if not pallet_design_data:
        ...
        if self.smart_data["z_angle"]:
            new_odd_poses = []
            for odd_pose in odd_poses:
                tf_tote_box = SE3([0,0,0,0,0,1,0])*SE3(odd_pose)
                new_odd_poses.append(tf_tote_box.xyz_quat)
                
            new_even_poses = []
            for even_pose in even_poses:
                tf_tote_box = SE3([0,0,0,0,0,1,0])*SE3(even_pose)
                new_even_poses.append(tf_tote_box.xyz_quat)  
            odd_poses = np.array(new_odd_poses)     
            even_poses = np.array(new_even_poses) 
