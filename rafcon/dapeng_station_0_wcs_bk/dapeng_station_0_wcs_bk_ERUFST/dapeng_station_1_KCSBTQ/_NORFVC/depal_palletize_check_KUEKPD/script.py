...
def all_direction_items(planning_env,container_items,current_direction):
    ...
    for direction,angle in all_direction_angle.items():
        ...
        tf_base_space = planning_env.get_workspace_ros("1").get_bottom_pose()    
        ...
        #删除环境箱子，并添加新的箱子   
        new_planning_env.clear_container_all_items("1")
        new_planning_env.add_container_items("1",new_container_items)  
...
def execute(self, inputs, outputs, gvm):
    ...
    #获取位置ID
    check_pose_dict = inputs["pose_dict"]
    #当前缓存区托盘料箱ID
    cache_pallet_tote_data = inputs["cache_pallet_tote_data"]
    new_pose_dict = {}

    #更新缓存区托盘位置ID信息
    for box_id,value in cache_pallet_tote_data.items():
        new_pose_dict[int(box_id)] = check_pose_dict[int(box_id)]    
        
    #获取缓存区托盘所有箱子
    pick_container_items = planning_env.get_container_items("1")

    #获取缓存区托盘坐标
    pick_workspace_ros = planning_env.get_workspace_ros("1")
    tf_base_pick_space = pick_workspace_ros.get_bottom_pose()
    
    if len(cache_pallet_tote_data)!=len(pick_container_items):
        self.logger.info("缓存区托盘上物料数量和视觉上物料数量不一致")
        raise "缓存区托盘上物料数量和视觉上物料数量不一致"
    ...
    for container_item in pick_container_items:
        ...
        if box_id in cache_pallet_tote_data.keys():
            ...
            #偏置
            tf_base_box_real = SE3([0.004,-0.0015,0,0,0,0,1])*tf_base_box_real
            if row ==9 and row_id in [5,6]:
                tf_base_box_real = SE3([0.0025,0,0,0,0,0,1])*tf_base_box_real    
            container_item.origin = Pose(*tf_base_box_real.xyz_quat)                           
        else:
    ...
    #获取缓存区托盘上放置规划箱子
    pick_plan_items = planning_env.get_unfinished_planned_items("1")  
    #获取所有朝向的放置规划,并根据当前朝向更新
    all_direction_plan_items = all_direction_items(planning_env,pick_plan_items,"C")
    pick_plan_items = all_direction_plan_items["C"]["container_items"]
    ...
    for container_item in pick_container_items:
        ...
        if not remove_plan_item:
            self.logger.info(f"缓存区托盘上没有找到匹配的箱子{container_item.name}")
            raise f"缓存区托盘上没有找到匹配的箱子"
    ...
    if len(check_plan_list)!=len(pick_container_items):
        self.logger.info(f"缓存区托盘放置规划匹配到{len(check_plan_list)}个箱子,实际视觉有{len(pick_container_items)}个箱子")   
        raise f"缓存区托盘放置规划匹配箱子和实际视觉箱子数量不匹配"
    
                    
    clear_container_all_items("1")
    add_container_items("1",pick_container_items)
    set_planned_items("1",new_pick_plan_items,[])
    return "success"
