...
def execute(self, inputs, outputs, gvm):
    ...
    for container_item in pick_container_items:
        ...
        if box_id in pallet_tote_data.keys():
            ...
            if row==5:
                tf_base_box_real = SE3([0.00,0.00,0,0,0,0,1])*tf_base_box_real
                row_id = int(box_id)%row
                lay_id = int(box_id)//row
                if row_id in [0]:
                    tf_base_box_real = tf_base_box_real*SE3([0.002,0,0,0,0,0,1]) 
                elif row_id in [1]:    
                    tf_base_box_real = tf_base_box_real*SE3([0.002,0,0,0,0,0,1])     
                elif row_id in [2]:    
                    tf_base_box_real = tf_base_box_real*SE3([0.002,0,0,0,0,0,1])   
    ...
    if len(bottom_container_items)==row or not bottom_container_items:
        ...
        if init_collision_items:
            ...
            #添加偏移点
            slide_list = [(x/200, y/200) for x in range(1, 18) for y in range(1, 18)]  
            slide_list+= [(-x/200, y/200) for x in range(1, 18) for y in range(1, 18)] 
            slide_list+= [(x/200, -y/200) for x in range(1, 18) for y in range(1, 18)] 
            slide_list+= [(-x/200, -y/200) for x in range(1, 18) for y in range(1, 18)] 
