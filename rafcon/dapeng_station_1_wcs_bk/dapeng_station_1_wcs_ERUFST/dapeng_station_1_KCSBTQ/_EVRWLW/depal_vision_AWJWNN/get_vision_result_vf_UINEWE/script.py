...
def execute(self, inputs, outputs, gvm):
    ...
    if mode=="RELEASE" or (mode=="DEBUG" and data_source_path==""):
        ...
        def build_primitive(info,pose_dict,pallet_tote_data):
            global send_error_msg
        ...
        for name,pose_dict in all_pose_dict.items():
            container_items = [build_primitive(info, pose_dict, pallet_tote_data) for info in results]
        ...
        if name != "C":
            self.logger.info(f"实际转向不为C面")  
            send_error_msg+=f"实际转向不为C面"   
            error_info = {
                "error": '10000',
                "data":{
                "error_msg": send_error_msg,
