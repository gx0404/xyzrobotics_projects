...
def check_pick_slide_move(checker,kinematic_solver,joints):
    use_slide = None
    slide_list = [[0.052, 0.052, 0.1, 0, 0, 0, 1], [0.052, -0.052, 0.1, 0, 0, 0, 1], [-0.052, 0.052, 0.1, 0, 0, 0, 1], [-0.052, -0.052, 0.1, 0, 0, 0, 1]]
    joints = kinematic_solver.convert_six_dof_to_four(joints)
    tf_map_flange_list_out = kinematic_solver.compute_fk(joints)
    for slide in slide_list:
        tf_map_flange_silde = SE3(slide)*tf_map_flange_list_out
         ...
         # 判断箱子是否抓取干涉
            ...
            if pick_collision_flag:
                ...
                for plan_item in remaining_plan_items:
                    #获取到放置是否干涉
                    place_collision_flag,change_180_no_collision = self.check_place_collision(our_robot,check_robot_list,init_planning_env,pick_joints,plan_item) 
               
                    if not place_collision_flag:
                        cost_item = CostItem(self.cost_estimate(plan_item,0),[[pick_item,plan_item,False]],[plan_item.name]) 
                        open_set.put(cost_item)    
                    if change_180_no_collision:
                        cost_item = CostItem(self.cost_estimate(plan_item,0)+20,[[pick_item,plan_item,True]],[plan_item.name]) 
                        open_set.put(cost_item)                                     
            ...
            for pick_item in remaining_pick_items:
                ...
                if pick_collision_flag:
                    ...
                    for plan_item in new_remaining_plan_items:
                        ...
                        place_collision_flag,change_180_no_collision = self.check_place_collision(our_robot,check_robot_list,check_planning_env,pick_joints,plan_item) 
                        ...
                        if change_180_no_collision:
                            ...
                            cost_item = CostItem(f_cost+self.cost_estimate(plan_item,cost_z)+20,new_path,new_state_key) 
                            open_set.put(cost_item)                                
    ...
    if not container_items:
        ...
        if not path:
            ...
            for pick_place_plan in path:
                pick_box_id = pick_place_plan[0].additional_info.values[-3]
                place_box_id = pick_place_plan[1].additional_info.values[-3]
                merge_2.append([pick_box_id,place_box_id,pick_place_plan[2]]) 
                #更新放置规划的箱子
    ...
    if not container_items:
        ...
        if not path:
            ...
            for pick_place_plan in path:
                pick_box_id = pick_place_plan[0].additional_info.values[-3]
                place_box_id = pick_place_plan[1].additional_info.values[-3]
                merge_1.append([pick_box_id,place_box_id,pick_place_plan[2]]) 
                #更新放置规划的箱子
                plan_items.remove(pick_place_plan[1])  
