...
def execute(self, inputs, outputs, gvm):
    ...
    #笼车缓存数据更新
    #校验数据
    if place_box_id in pallet_tote_data_2.keys():
        self.logger.info(f"此次放置位置号:{place_box_id}与笼车缓存缓存区已有位置号重复")    
        raise f"此次放置位置号:{place_box_id}与笼车缓存缓存区已有位置号重复"  
    if len(place_container_items)!=len(pallet_tote_data_2)+1:
        self.logger.info(f"笼车缓存数据与当前环境数量长度不一致")
        raise f"笼车缓存数据与当前环境数量长度不一致"
    #找到此次放置的箱子位置信息
    ...
    pallet_tote_data_2[place_box_id] = {
        "barcode":barcode,
