...
def execute(self, inputs, outputs, gvm):
    ...
    if not set_io_port:
        ...
        if 2 not in set_io_port or 14 not in set_io_port:
            self.logger.info("Set IO not port 2,14,wait 4,3")
            port_ids+=[4,3]

        if 3 not in set_io_port or 15 not in set_io_port:
            self.logger.info("Set IO not port 3,15,wait 7,8")
            port_ids+=[7,8]

        if 4 not in set_io_port or 16 not in set_io_port:
            self.logger.info("Set IO not port 4,16,wait 5,6")
            port_ids+=[5,6]
