...
def execute(self, inputs, outputs, gvm):
    ...
    #获取缓存区上放置规划箱子
    plan_items = planning_env.get_unfinished_planned_items("1")  
    ...
    for container_item in container_items:
        ...
        remove_plan_item = list(filter(lambda x:np.allclose(pose_to_list(x.origin)[0:2],tf_box_real.xyz_quat[0:2],atol=0.08)\
            and np.allclose(pose_to_list(x.origin)[2],tf_box_real.xyz_quat[2],atol=0.05),plan_items))
    ...
    #耦合抓取箱子和放置规划的箱子
    bottom_plan_items = filter_bottom_items(plan_items)
    bottom_container_items = filter_bottom_items(container_items)
    
    init_no_overlap_plan_items = []
    row = gvm.get_variable("row", per_reference=False, default=None)
    if len(bottom_container_items)==row:
        update_plan_items = []
        ...
        if init_collision_items:
            ...
            #添加偏移点
            slide_list = [(x/200, y/200) for x in range(1, 16) for y in range(1, 16)]  
            slide_list+= [(-x/200, y/200) for x in range(1, 16) for y in range(1, 16)] 
            slide_list+= [(x/200, -y/200) for x in range(1, 16) for y in range(1, 16)] 
            slide_list+= [(-x/200, -y/200) for x in range(1, 16) for y in range(1, 16)] 
            ...
            #生成所有碰撞箱子排列组合
            """尝试改动"""
            #all_collision_items_tupple = list(permutations(init_collision_items))
            ...
            break_flag = False
            #排列组合依次做aabb碰撞检测偏移
            for index,collision_items in enumerate(all_collision_items_list):
                if break_flag == True:
                    break
                stack = Stack()
    ...
    for plan_item in plan_items:
        tf_space_box_real = (tf_base_place_space.inv())*SE3(pose_to_list(plan_item.origin))
    ...
    add_container_items("1",container_items)
