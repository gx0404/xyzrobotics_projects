...
def execute(self, inputs, outputs, gvm):
    ...
    if inputs["grasp_plan"]:
        ...
        self.logger.info("判断是否需要翻转180碰撞检测")
        ...
        if not (place_id =="2" or place_id =="3"):
            self.logger.info("非笼车放置位,不做翻转180碰撞检测")
            return "fail"
        else:
            if not sku_dimension==[0.4,0.3,0.23]:
                self.logger.info("非中欧箱型,不做翻转180碰撞检测")
                return "fail"    
