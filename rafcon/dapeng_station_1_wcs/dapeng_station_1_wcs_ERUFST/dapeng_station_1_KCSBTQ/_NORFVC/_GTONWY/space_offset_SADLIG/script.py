...
def execute(self, inputs, outputs, gvm):
    ...
    if row==5:
        ...
        outputs["place_drop_buffer"] = 0.022
    elif row==9:
    ...
    if object_poses:
        ...
        if place_container_items:
            ...
            if pick_check_items[0].origin.z > place_check_items[0].origin.z:
                space_pose_obj_z = pick_check_items[0].origin.z+tip_pose_z+self.smart_data["sku_max_height"]+0.1   
            else:
                space_pose_obj_z = place_check_items[0].origin.z+tip_pose_z+self.smart_data["sku_max_height"]+0.1   
        else:
            space_pose_obj_z = pick_check_items[0].origin.z+tip_pose_z+self.smart_data["sku_max_height"]+0.1                         
