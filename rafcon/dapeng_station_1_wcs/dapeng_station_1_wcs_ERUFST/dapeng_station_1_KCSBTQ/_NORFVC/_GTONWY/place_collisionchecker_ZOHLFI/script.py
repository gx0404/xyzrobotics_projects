...
def execute(self, inputs, outputs, gvm):
    ...
    #计算缩小6轴极限
    first_robot_msg = check_our_robot.to_ros_msg()
    first_robot_msg.joint_limits[3].min_position += 0.04
    first_robot_msg.joint_limits[3].max_position -= 0.04
    check_our_robot = RobotRos.from_ros_msg(first_robot_msg)
   ...
   #相机扩大一点
    for i in init_clamp_collision:
        if i.name == "body":
            i.primitives[0].dimensions = [0.378, 0.578, 0.65]  
    ...
    # our_robot_msg.tool.tool_collisions.primitives.append(GeometricPrimitive(type=GeometricPrimitive.BOX, dimensions=[0.55, 0.33, 0.2], relative_pose=Pose(
    # -0.352-0.209/2, 
    # 0, 
    # 0.0835, 
    # 0, 0, 
    # 0.707, 
    # 0.707)))   
    # our_robot_msg.tool.tool_collisions.primitives.append(GeometricPrimitive(type=GeometricPrimitive.BOX, dimensions=[0.38, 0.75, 0.3], relative_pose=Pose(
    # -0.084,
    # 0,
    # 0.08+0.3/2+0.07,
    # -0.0024977710695978537,
    # -0.0030570717793057247,
    # 0.70710678,
    # 0.70710678)))    
       
    # length = 0.524
    # length_x = 0.013
    # if "+y" in un_clamp_collision_name and "-y" not in un_clamp_collision_name:     
    #     width = 0.474+0.04
    #     width_y = 0.042
    # elif "+y" not in un_clamp_collision_name and "-y" in un_clamp_collision_name:
    #     width = 0.474+0.04
    #     width_y = -0.042
    # elif "+y" in un_clamp_collision_name and "-y" in un_clamp_collision_name:
    #     width = 0.558+0.01 
    #     width_y = 0.0
    # else:
    #     raise "无效的挡板碰撞体"
                             
    # our_robot_msg.tool.tool_collisions.primitives.append(GeometricPrimitive(type=GeometricPrimitive.BOX, dimensions=[width, length, 0.7], relative_pose=Pose(
    # length_x
    # width_y,
    # 0.08+0.7/2+0.07,
    # -0.0,
