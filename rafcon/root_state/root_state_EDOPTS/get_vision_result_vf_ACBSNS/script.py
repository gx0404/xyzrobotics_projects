...
def execute(self, inputs, outputs, gvm):
    ...
    old_container_items = planning_env.get_container_items(workspace_id)
    for item in old_container_items:
        self.logger.info(f"原先放置箱子坐标:{pose_to_list(item.origin)}")
    ...
    if sku_dimension==[0.405,0.303,0.16]:
        ...
        if eye_in_hand:
            vision_service = "calculate_object_poses_0_2"    
            capture = "capture_images_1"
    elif sku_dimension==[0.4,0.3,0.23]:
        capture = "capture_images"
        vision_service = "calculate_object_poses_1_1"
        if eye_in_hand:
            vision_service = "calculate_object_poses_1_2"  
            capture = "capture_images_1"          
    elif sku_dimension==[0.6,0.4,0.23]:
        vision_service = "calculate_object_poses_2_1"
        capture = "capture_images"
        if eye_in_hand:
            vision_service = "calculate_object_poses_2_2"     
            capture = "capture_images_1"       
    else:
        raise "无效的尺寸"      
    ...
    if flag_capture_image:
        ...
        while not self.preempted:
            if eye_in_hand:
                ...
                capture_res = vision_bridge.run(int(vision_id), capture,info=json.dumps({"tf_world_hand":scan_pose}))
            else:
                capture_res = vision_bridge.run(int(vision_id), capture)                  
    ...
    def build_primitive(info,pose_dict,cache_pallet_tote_data):
        ...
        # if "to_ws" in pg.additional_info.keys:
        #     index = pg.additional_info.keys.index("to_ws")
        #     pg.additional_info.values[index] = ""    
        # else:    
