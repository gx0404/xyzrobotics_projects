...
def execute(self, inputs, outputs, gvm):
    ...
    if mode=="RELEASE" or (mode=="DEBUG" and data_source_path==""):
        ...
        ftr = vision_bridge.async_run(int(vision_id), "merge_cloud_1")    
        ftr.get()
        ...
        def build_primitive(info,pose_dict,pallet_tote_data_2):
            ...
            if box_id in pallet_tote_data_2.keys():
                ...
                pg.additional_info.values.append(pallet_tote_data_2[box_id]["barcode"]) 
            else:
        ...
        if len(results)!=len(pallet_tote_data_2):
            raise "视觉生成的箱子数量和数据库箱子数量不一致"
        container_items = [build_primitive(info, pose_dict, pallet_tote_data_2) for info in results]
