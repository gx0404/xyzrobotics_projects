...
from xyz_env_manager.client import get_planning_environment,modify_primitive_group_of_environment,modify_workspace_of_environment
...
def execute(self, inputs, outputs, gvm):
    ...
    for collision_objcet in pl.collision_objects:
        if collision_objcet.name in collision_pallet_list:
            ...
            collision_objcet.origin.z=-1.2+1.7/2-1
            modify_primitive_group_of_environment(collision_objcet)    
            
    planning_env = PlanningEnvironmentRos.from_ros_msg(pl)
    unfinish_planned_items_2 = planning_env.get_unfinished_planned_items("2")
    if not unfinish_planned_items_2:
        self.logger.info(f"2号空间无放置规划,需要更新环境")
        pl_workspace_2 = pl.workspaces[2]
        pl_workspace_2.bottom_pose.z = -1.121
        pl_workspace_2.dimensions = (1.25, 1.05, 1.62)   
        pl_workspace_2.pallet.length = 1.25
        pl_workspace_2.pallet.width = 1.05
        pl_workspace_2.pallet.top_origin.z = -1.121-0.05
        modify_workspace_of_environment(pl_workspace_2)
        
    unfinish_planned_items_3 = planning_env.get_unfinished_planned_items("3")
    if not unfinish_planned_items_3:    
        pl_workspace_3 = pl.workspaces[3]   
        pl_workspace_3.bottom_pose.z = -1.121  
        pl_workspace_3.dimensions = (1.25, 1.05, 1.62)    
        pl_workspace_3.pallet.length = 1.25
        pl_workspace_3.pallet.width = 1.05   
        pl_workspace_3.pallet.top_origin.z = -1.121-0.05 
    ...
    # pl_workspace_1 = pl.workspaces[1]
    # pl_workspace_1.bottom_pose.z = -1.121
    # pl_workspace_1.pallet.top_origin.z = -1.121    
    # modify_workspace_of_environment(pl_workspace_1) 
    ...
    gvm.set_variable("conveyor_pallet", 7, per_reference=False)
    gvm.set_variable("motion_payload", None, per_reference=True)  
      
    outputs["sku_info"] = sku_info_default    
    import copy
    sku_init_default = copy.deepcopy(sku_info_default) 
    sku_init_default["length"] = 0.4
    sku_init_default["width"] = 0.3 
    outputs["init_sku_info"] = sku_init_default
    return "success"