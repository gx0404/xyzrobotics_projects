...
def execute(self, inputs, outputs, gvm):
    ...
    for check_slide in check_slide_list:
        ...
        for check_slide_xy in check_offset_xy_list:
            ...
            if checker.check_point_collision(check_slide_joint):
                self.logger.info(f"后验XY偏移点{check_slide_xy}与抓取碰撞")
                collision_flag = True
                break   
    ...
    if not use_slide_list:
        raise "后验XY偏移点检测无解"
    self.logger.info(f"use slide为{use_slide_list}")               
    ...
    for use_slide_check in use_slide_list:
        ...
        for check_slide in offset_check_slide_list:
            ...
            checker = CollisionChecker(check_slide_robot, planning_env)
            if checker.check_point_collision(check_slide_joint):
                self.logger.info(f"后验偏移点Z{check_slide}与抓取碰撞")
                z_check_collision = True
                break
        if z_check_collision:
            self.logger.info(f"后验偏移点Z{use_slide_check}与抓取碰撞")
            continue
        else:
            use_slide = use_slide_check
            self.logger.info(f"后验偏移点Z{use_slide_check}与抓取无碰撞")
            break
    if not use_slide:
        self.logger.info(f"后验偏移点Z无解")
        raise "后验偏移点Z无解"
    if use_slide[0]>0:
        use_slide[0] = 0.06
    else:
