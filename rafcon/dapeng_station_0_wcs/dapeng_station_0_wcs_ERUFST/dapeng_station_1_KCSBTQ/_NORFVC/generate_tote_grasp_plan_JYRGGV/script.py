...
def execute(self, inputs, outputs, gvm):
    ...
    #通过抓取计算得到的路径，指定抓取箱子
    restore_depal_path = gvm.get_variable("restore_depal_path", per_reference=False, default=None)
    pick_item_id = restore_depal_path[0]
    ...
    def filter_plan_items(slots):
        new_slots = []
        for slot in slots:
            place_box_id = slot.additional_info.values[-3]
            if int(place_box_id)%4!=1:
                new_slots.append(slot)
        return new_slots

    #slots = filter_plan_items(slots)        
    ...
    pick_id = items[0].additional_info.values[-3]
    place_id = slots[0].additional_info.values[-3]
    outputs["pick_box_id"] = pick_id
    outputs["place_box_id"] = place_id
    ...
    self.logger.info(f"grasp plan 已规划出指定箱子{restore_depal_path[0]},更新路径")
    restore_depal_path.pop(0)
    gvm.set_variable("restore_depal_path", restore_depal_path, per_reference=False)
