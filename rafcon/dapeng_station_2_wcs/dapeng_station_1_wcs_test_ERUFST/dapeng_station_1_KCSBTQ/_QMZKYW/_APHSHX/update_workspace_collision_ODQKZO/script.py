...
def execute(self, inputs, outputs, gvm):
    ...
    def update_planning_collision(grasp_plan):
        ...
        check_padding_list = [f"workspace_{i}_full_padding_pg" for i in range(5,6)]
        for collision_object in planning_env_msg.collision_objects:
            if collision_object.name in check_padding_list:
                dimensions = list(collision_object.primitives[0].dimensions)
                enlarge_width = 0.25
                dimensions[1]+=enlarge_width
                
                enlarge_length = 2.5
                dimensions[0]+=enlarge_length
                collision_object.origin.x-=enlarge_length/2
                
                dimensions[2]+=0.5
                collision_object.primitives[0].dimensions=tuple(dimensions)
                
                append_primitive = GeometricPrimitive(type=GeometricPrimitive.BOX, 
                                                      dimensions=[1.205, 1.05+0.7, 2.3], relative_pose=Pose(
                                                        enlarge_length/2,
                                                        0+0.7/2,
                                                        0.825,
                                                        0.0,
        ...
        check_padding_list = [f"workspace_{i}_full_padding_pg" for i in range(7,8)]
        for collision_object in planning_env_msg.collision_objects:
            if collision_object.name in check_padding_list:
                dimensions = list(collision_object.primitives[0].dimensions)
                dimensions[0]+=0.1
                collision_object.origin.x-=0.1/2
                collision_object.primitives[0].dimensions=tuple(dimensions)                                                                   
    ...
    def update_visual_collision(trajectory):
        ...
        check_padding_list = [f"workspace_{i}_full_padding_pg" for i in range(7,9)]
        for collision_object in planning_env_msg.collision_objects:
            if collision_object.name in check_padding_list:
                dimensions = list(collision_object.primitives[0].dimensions)
                dimensions[0]+=0.1
