...
def execute(self, inputs, outputs, gvm):
   ...
   plan_items = filter(lambda item: item.additional_info.values[-3] in history_path, plan_items)
   ...
   if len(collision_items):
      ...
      #得到路径   
      restore_depal_path = calculation_dict["path"] 
            
      #路径
      if not restore_depal_path:
         self.logger.info(f"没有找到可以抓取到目标物无碰撞的路径解")
         raise Exception("当前路径所有箱子已遍历,但仍未找到目标点,是个bug")
         
      self.logger.info(restore_depal_path)

      outputs["restore_depal_path"] = restore_depal_path
